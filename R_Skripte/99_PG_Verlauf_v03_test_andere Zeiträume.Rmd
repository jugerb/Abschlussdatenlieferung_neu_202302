...
tmp <- pg %>%
  ...
  mutate(
    ...
    Apr20 = ifelse(any(as.Date('2020-04-01') %within% int), V_PFLEGEGRAD[as.Date('2020-04-01') %within% int], Beginn),
    Jul20 = ifelse(any(as.Date('2020-07-01') %within% int), V_PFLEGEGRAD[as.Date('2020-07-01') %within% int], Apr20),
    Okt20 = ifelse(any(as.Date('2020-10-01') %within% int), V_PFLEGEGRAD[as.Date('2020-10-01') %within% int], Jul20),
    Jan21 = ifelse(any(as.Date('2021-01-01') %within% int), V_PFLEGEGRAD[as.Date('2021-01-01') %within% int], Okt20),
    Apr21 = ifelse(any(as.Date('2021-04-01') %within% int), V_PFLEGEGRAD[as.Date('2021-04-01') %within% int], Jan21),
    Jul21 = ifelse(any(as.Date('2021-07-01') %within% int), V_PFLEGEGRAD[as.Date('2021-07-01') %within% int], Apr21),
    Okt21 = ifelse(any(as.Date('2021-10-01') %within% int), V_PFLEGEGRAD[as.Date('2021-10-01') %within% int], Jul21),
    Ende = ifelse(any(as.Date('2021-12-31') %within% int), V_PFLEGEGRAD[as.Date('2021-12-31') %within% int], Okt21)
    ) %>%
  ungroup() %>%
...
long <- dat %>%
  ...
  mzp = factor(mzp, levels = c('Beginn', 'Apr20', 'Jul20', 'Okt20', 'Jan21', 'Apr21', 'Jul21', 'Okt21', 'Ende'))
  ) 
...
long <- long %>%
  ...
  mutate(mzp_date = as.Date(rep(c('2020-01-01', '2020-04-01', '2020-07-01', '2020-10-01', 
                                  '2021-01-01', '2021-04-01', '2021-07-01', '2021-10-01', '2021-12-31'), nrow(long)/9))) %>% 
...
tmp <- long %>%
  ...
  tally() %>% 
  rename(freq = n) %>%
  mutate(mzp_fct = factor(mzp, levels=c('Beginn', 'Apr20', 'Jul20', 'Okt20', 'Jan21', 'Apr21', 'Jul21', 'Okt21', 'Ende')),
         pg3 = factor(pg3)) %>% 
  complete(pg3) %>% 
...
bmp(file="O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Querschnitt_gesamt_3Monate.bmp", 
...
bmp(file="O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Querschnitt_gesamt_3Monate_rel.bmp", 
    width = 750, height = 500, type="windows")
print(p)
dev.off()
```
...
for (tg in tgs) {
  ...
  # Plot exportieren
  bmp(paste(paste("O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Querschnitt_3Monate", tg, sep='_'), '.bmp', sep=''),
      width = 750, height = 500, type="windows")
  print(p1)
  dev.off()
  ...
  # Plot exportieren
  bmp(paste(paste("O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Querschnitt_3Monate_rel", tg, sep='_'), '.bmp', sep=''),
      width = 750, height = 500, type="windows")
  print(p1)
  dev.off()
...
write.xlsx(list_of_datasets, 
           file = "O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/03_Ergebnisse/99_Querschnitt_PG_2020_21_3Monate.xlsx")
```
...
tmp <- dat %>% 
  mutate(
    ...
    Apr20_ch = Apr20 - Beginn,
    Jul20_ch = Jul20 - Apr20,
    Okt20_ch = Okt20 - Jul20,
    Jan21_ch = Jan21 - Okt20,
    Apr21_ch = Apr21 - Jan21,
    Jul21_ch = Jul21 - Apr21,
    Okt21_ch = Okt21 - Jul21,
    Ende_ch = Ende - Okt21,
    
    # Veränderungen kategorisieren (alt-Verstorbene werden jeweils entfernt)
    Apr20 = case_when(
      Apr20_ch > 90 ~ 'Verstorben', 
      Apr20_ch < 0 ~ 'zurückgestuft', 
      Apr20_ch > 0 ~ 'hochgestuft', 
      Apr20_ch == 0 ~ 'unverändert'
    ),
    Jul20 = case_when(
    ...
    Okt20 = case_when(
      Okt20_ch > 90 ~ 'Verstorben', 
      Okt20_ch < 0 ~ 'zurückgestuft', 
      Okt20_ch > 0 ~ 'hochgestuft', 
      Okt20_ch == 0 ~ 'unverändert'
    ), 
    Jan21 = case_when(
    ...
    Apr21 = case_when(
      Apr21_ch > 90 ~ 'Verstorben', 
      Apr21_ch < 0 ~ 'zurückgestuft', 
      Apr21_ch > 0 ~ 'hochgestuft', 
      Apr21_ch == 0 ~ 'unverändert'
    ),
    Jul21 = case_when(
    ...
    Okt21 = case_when(
      Okt21_ch > 90 ~ 'Verstorben', 
      Okt21_ch < 0 ~ 'zurückgestuft', 
      Okt21_ch > 0 ~ 'hochgestuft', 
      Okt21_ch == 0 ~ 'unverändert'
    ),
    Ende = case_when(
...
# pivotieren & in Faktoren umwandeln + umbenennen
long <- tmp %>%
  select(V_ID, TG, V_STERBEDATUM, int, n, Apr20:Ende) %>% 
  pivot_longer(Apr20:Ende, names_to = 'mzp', values_to = 'pg_change') %>%
  group_by(mzp, pg_change) %>% 
  ungroup() %>% 
  mutate(
    pg_change = as.factor(pg_change),
    mzp = factor(mzp, levels = c('Apr20', 'Jul20', 'Okt20', 'Jan21', 'Apr21', 'Jul21', 'Okt21', 'Ende'))
  )
...
long <- long %>%
  ...
  mutate(
    # exaktes Datum des Querschnitts hinzufügen
     mzp_date = as.Date(rep(c('2020-04-01', '2020-07-01', '2020-10-01', '2021-01-01',
                              '2021-04-01', '2021-07-01', '2021-10-01', '2021-12-31'), nrow(long)/8)),  
    # Längsschnitt-Beobachtungsinterval hinzufügen
    int_mzp = case_when( 
      mzp == 'Apr20' ~ interval(start=as.Date('2020-01-01') , end=as.Date('2020-03-31')),
      mzp == 'Jul20' ~ interval(start=as.Date('2020-04-01') , end=as.Date('2020-06-30')),
      mzp == 'Okt20' ~ interval(start=as.Date('2020-07-01') , end=as.Date('2020-09-30')),
      mzp == 'Jan21' ~ interval(start=as.Date('2020-10-01') , end=as.Date('2020-12-31')),
      mzp == 'Apr21' ~ interval(start=as.Date('2021-01-01') , end=as.Date('2020-03-31')),
      mzp == 'Jul21' ~ interval(start=as.Date('2021-04-01') , end=as.Date('2021-06-30')),
      mzp == 'Okt21' ~ interval(start=as.Date('2020-07-01') , end=as.Date('2020-09-30')),
      mzp == 'Ende' ~ interval(start=as.Date('2021-10-01') , end=as.Date('2021-12-31')), 
      TRUE ~ as.interval(NA)
    ),
...
tmp <- long %>%
  ...
  tally() %>% 
  rename(freq = n) %>%
  mutate(mzp_fct = factor(mzp, levels=c('Apr20', 'Jul20', 'Okt20', 'Jan21', 'Apr21', 'Jul21', 'Okt21', 'Ende')),
...
bmp(file="O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Längsschnitt_gesamt_3Monate.bmp", 
    width = 750, height = 500, type="windows")
print(p)
dev.off()
...
bmp(file="O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Längsschnitt_gesamt_3Monate_rel.bmp", 
    width = 750, height = 500, type="windows")
print(p)
dev.off()
```
...
tmp <- long %>%
  ...
  ungroup() %>%
  mutate(
    mzp_fct = factor(mzp, levels=c('Apr20', 'Jul20', 'Okt20', 'Jan21', 'Apr21', 'Jul21', 'Okt21', 'Ende')),
    pg_change2 = factor(pg_change2, levels=c('zurückgestuft', 'unverändert', 'hochgestuft', 'Verstorben')),     
    TG = factor(TG)
    ) %>% 
...
for (tg in tgs) {
  ...
  # Plot exportieren
  bmp(paste(paste("O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Längsschnitt_3Monate", tg, sep='_'), '.bmp', sep=''),
      width = 750, height = 500, type="windows")
  print(p1)
  dev.off()
  ...
  # Plot exportieren
  bmp(paste(paste("O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/04_Plots/Längsschnitt_3Monate_rel", tg, sep='_'), '.bmp', sep=''),
      width = 750, height = 500, type="windows")
  print(p1)
  dev.off()
...
```{r pg_change_export}
list_of_datasets <- list("pg_change_gesamt" = pg_change_gesamt, "pg__change_tg" = pg_change_tg)

write.xlsx(list_of_datasets, 
           file = "O:/U5279/Routinedaten/Abschlussdatenlieferung_neu_202302/Daten/03_Ergebnisse/99_Längsschnitt_PG_2020_21_3Monate.xlsx")
```