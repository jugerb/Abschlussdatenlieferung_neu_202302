...
**PROBLEM?**: ich finde drei Personen weniger aus HR als nach Pauls Datenzuschnitt (vermutlich zunächst in einer anderen TG)
```{r test}
...
# Dauer in PG
pg <- pg %>% 
  mutate(int=interval(start = pg$V_PG_VON2, end=pg$V_PG_BIS2) / ddays(1) +1) 
...
qualy <- full_join(qualy, tmp2[c('TG', 'PG', 'real_d')], by = c('TG', 'PG')) %>% 
  arrange(TG, match(PG, c('PG1', 'PG2', 'PG3', 'PG4', 'PG5', 'Verstorben'))) %>% 
...
qualy <- qualy %>% 
  ...
  mutate(real_weighted_d = real_d * weight) %>%
  group_by(TG) %>% 
  ...
  mutate(sum_weighted_max_d = sum(weighted_max_d), 
         sum_weighted_real_d = sum(real_weighted_d)) %>% 
  ungroup() %>% 
  # Anteil tatsächlich erreichter QUALYs pro TG
  mutate(perc_qualy = sum_weighted_real_d / sum_weighted_max_d)
```
...
```{r export}
write.xlsx(qualy, 
           file = "N:/Pool/U5200/U5279/ap/AP700 Berichtlegung/Abschlussbericht/qualy_analysis_20230331.xlsx", 
           overwrite=FALSE)
```
