...
hilfsdatei_hkp <- read_excel("O:/U5279/Routinedaten/Abschlussdatenlieferung20_21/Daten/hilfsdatei_hkp.xlsx")
```
...
data2$LEISTUNG_VON <- as.Date(data2$LEISTUNG_VON, tryFormats = c("%Y-%m-%d"))
data3c <- left_join(data2, data3b, by=c("V_ID","LEISTUNG_VON"="day"))
...
data1a_zg3_verweil <- data1a_zg3_TG %>% group_by(TG) %>% 
...
data4_zg3_id <- data4 %>%  group_by(V_ID,TG, HKP_GOP) %>% 
...
data4_zg3_tage1 <- data4 %>%  group_by(V_ID,TG, LEISTUNG_VON, HKP_GOP) %>% 
  summarise(ID_ja = 1) %>% group_by(TG, HKP_GOP ) %>% 
...
# alle Kosten
data4_zg3_kosten <- data4  %>% group_by( TG, HKP_GOP) %>%  
...
data4_zg3_c <- left_join(data4_zg3_b, data4_zg3_kosten, by=c("TG","HKP_GOP"))   %>% mutate_all(~replace(., is.na(.), 0)) %>% relocate(HKP_GOP, .before = VZ_tage) 

# Daten für Tabelle vorbereiten
results_alle <- data4_zg3_c %>% mutate(IN_Tage_HKP_jahr= IN_Tage_HKP*365/oKH_Tage,
...
data4_zg3_id <- data4 %>%  group_by(V_ID,TG, HKP_GOP,V_PFLEGEGRAD,V_ALTER, tod) %>% 
  summarise(HKP_ja_id = 1) %>% 
  group_by(TG, HKP_GOP,V_PFLEGEGRAD,V_ALTER, tod) %>% 
  summarise(n_IN_HKP = sum(HKP_ja_id))

data4_zg3_id2 <- data1 %>%  group_by(V_ID,TG, V_PFLEGEGRAD,V_ALTER, tod) %>% 
...
data4_zg3_tage1 <- data4 %>%  group_by(V_ID,HKP_GOP,LEISTUNG_VON,TG,V_PFLEGEGRAD, V_ALTER, tod) %>% 
  summarise(ID_ja = 1) %>% group_by(HKP_GOP,TG,  V_PFLEGEGRAD,V_ALTER, tod ) %>% 
  summarise(IN_Tage_HKP= sum(ID_ja))

data4_zg3_tage2<-  left_join(data1a_zg3_verweil,data4_zg3_tage1, by=c("HKP_GOP","TG","V_PFLEGEGRAD","V_ALTER", "tod")  ) %>% 
  mutate_all(~replace(., is.na(.), 0)) %>% 
  mutate(noIN_Tage_HKP= oKH_Tage -IN_Tage_HKP) %>% 
  dplyr::select(-c(oKH_Tage, VZ_tage, KH_Tage))

# alle Kosten
data4_zg3_kosten <- data4 %>%  group_by(HKP_GOP,TG,V_PFLEGEGRAD,V_ALTER, tod ) %>%  
  summarise( SUM_HKP_GesKosten = sum(ZAHLBETRAG))
...
data4_zg3_c <- left_join(data4_zg3_b, data4_zg3_kosten, by=c("HKP_GOP","TG","V_PFLEGEGRAD", "V_ALTER", "tod") )   %>% 
  mutate_all(~replace(., is.na(.), 0)) %>% relocate(HKP_GOP, .before = VZ_tage) 

# Daten für Tabelle vorbereiten
results_alle_w <- data4_zg3_c %>% mutate(IN_Tage_HKP_jahr= IN_Tage_HKP*365/oKH_Tage,
                                       proz_n_IN_HKP= n_IN_HKP/n_ID,
                                       HKP_GesKosten_jahr= SUM_HKP_GesKosten*365/oKH_Tage)

# relevante Variablen für Tabelle
results_alle_w <- results_alle_w %>% dplyr::select(TG,V_PFLEGEGRAD,V_ALTER, tod, HKP_GOP,n_ID, VZ_tage, oKH_Tage, n_IN_HKP, IN_Tage_HKP, proz_n_IN_HKP, IN_Tage_HKP_jahr, HKP_GesKosten_jahr )
```
...
data5 <- results_alle_w %>% filter(HKP_GOP=="32301") %>%  group_by(TG) %>% 
...
# Wichtung nach TG1
data6 <- data5a %>% filter(TG=="TG1" &HKP_GOP=="32301") %>% mutate(weight=VZ_tage/days_tg) %>% ungroup() %>% 
...
#finale Werte
data7 <- data6b %>% group_by(TG,HKP_GOP) %>% 
 filter(weight>0) %>% 
  summarise(VZ_tage=sum(VZ_tage),
...
results_weight <- results_weight %>% dplyr::select(TG, HKP_GOP,N, VZ_tage,IN_Tage_HKP_jahr, HKP_GesKosten_jahr )
```
...
write.xlsx(list_of_datasets, file = "20_Inanspruch_HKP2.xlsx")
```