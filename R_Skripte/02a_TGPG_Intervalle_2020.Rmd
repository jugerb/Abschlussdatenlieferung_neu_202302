...
data4a <- data4a %>% 
  filter(n_ID==1) %>% 
  dplyr::select(V_ID,V_ALTER,V_STERBEDATUM, ZG3, ZG4)
...
data11 <- data11 %>% 
  dplyr::mutate(TG= if_else(tg==1, "TG1", if_else(tg==2, "TG3", if_else(tg==3, "TG2", if_else(tg==5, "TG5","TG6")))))

table(data11$tg, data11$TG)
