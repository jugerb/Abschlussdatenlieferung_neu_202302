...
# alle Ausschlussbedingungen, um Personen mit Intensvpflege zu entfernen
check3 <- check2 %>% 
  filter(HKP_GOP_3 == 'A' | 
           HKP_GOP %in% intensiv_voll | 
           HKP_GOP_456 %in% intensiv_voll | 
           HKP_GESKOSTEN < (-100))
...
help1a <- pg07 %>% arrange(V_ID) %>% filter(GEBPOS_KURZ_NEU=="Geld") #%>%slice_head(n = 10000)
...
# 
# # IDs mit Tageskosten von über xx ausschließen
# check5 <- left_join(data11, check4, by=c("V_ID"))
# 
# check6 <- check5 %>% filter(!(is.na(exkl)))
# 
# # Vergleich mit ganzem Datensatz
# table(check6$TG)
# table(data11$TG)
# 
# check7 <- check6 %>% group_by(TG) %>%
#   summarise(tage= sum(intervalldauer))
# 
# table(check7$tage)
# 
